...
- **암묵적 요구사항 추론** — 이것이 핵심이다:
  ...
  - "오늘 날짜 열에 추가" → FIND_DATE_COLUMN + COPY_RANGE + 점 표기법 참조 필요. **date 파라미터에 `"today"` 금지 — 반드시 실제 날짜를 ISO 형식(`"YYYY-MM-DD"`)으로 입력**
...
다음 15개 값만 사용 가능하다. **정확히 대문자로 표기해야 한다**:

```
OPEN_FILE, READ_COLUMNS, READ_RANGE, WRITE_DATA, CLEAR_RANGE,
RECALCULATE, FIND_DATE_COLUMN, COPY_RANGE, SAVE_FILE, VALIDATE,
FORMAT_MESSAGE, SEND_MESSENGER, SEND_EMAIL, GENERATE_PPT, LOG
```
...
8. 모든 `action` 값이 15개 ActionType enum 중 하나인가?
