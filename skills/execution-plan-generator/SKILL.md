...
1, 2를 분석한 뒤, 반복 실행 시 **매번 달라질 수 있는 요소**를 스스로 추론한다.
추론한 가변 요소를 사용자에게 확인받은 뒤 plan을 생성한다.

**가변 요소 추론 기준:**
- "매일 데이터를 넣어", "반복 작업" → raw data 행 수가 가변적
- "일별/주별 보고" → 날짜 기준 위치가 가변적
- "파일을 저장해" → 파일명에 날짜 포함 가능 (가변)
- "여러 소스에서 읽어" → 소스별 데이터 범위가 가변적

**사용자 확인 질문 템플릿:**

추론을 마치면 아래 형식으로 사용자에게 확인한다:

```
[가변 데이터 확인]
분석 결과, 아래 데이터가 반복 실행 시 가변적일 수 있습니다:

1. ✅ raw data 행 수: 매일 데이터 건수가 다를 수 있음
2. ✅ 날짜 기준 컬럼 위치: 실행 날짜에 따라 쓰기 위치가 달라짐
3. ❓ 결과 파일명: 날짜별로 다른 파일명이 필요한가요?

위 내용이 맞는지, 추가로 가변적인 요소가 있는지 알려주세요.
```

사용자 확인이 없으면 plan을 생성하지 않는다. **"데이터 파일을 보여달라"고 하지 않는다.**
...
- **가변 데이터 식별** — 반복 실행 시 매번 달라지는 요소를 식별한다:
  - raw data 행 수가 가변적인가? (매일 건수가 다른 생산 데이터 등)
  - 날짜에 따라 읽기/쓰기 위치가 달라지는가?
  - 파일명에 날짜나 가변 요소가 포함되는가?
  - 식별된 가변 요소는 입력 수집 3단계에서 사용자에게 확인받는다
...
**가변 데이터 대응 규칙** — 입력 수집 3단계에서 가변으로 확인된 요소에 적용:

| 가변 요소 | 올바른 패턴 | 금지 패턴 |
|---------|-----------|----------|
| raw data 행 수 | `READ_COLUMNS`에서 **`end_row` 생략** (자동 감지) | `end_row: 100` 같은 고정값 ❌ |
| 쓰기 대상 행 수 | `WRITE_DATA` 사용 (source 길이만큼 자동 확장) | 고정 range로 쓰기 ❌ |
| 클리어 범위 | 충분히 큰 범위 지정 (예: `"B3:F100000"`) | 실제 데이터 행에 딱 맞추기 ❌ |
| 날짜 기준 위치 | `FIND_DATE_COLUMN`, `FIND_DATE_RANGE`, `FIND_ANCHOR` 사용 | 하드코딩 열 문자 (예: `"CP"`) ❌ |
| 파일명 날짜 | `EXTRACT_DATE` 또는 `dynamic_params` type=date 사용 | 날짜 문자열 하드코딩 ❌ |

**가변 행 워크플로우 패턴:**
```
READ_COLUMNS(end_row 생략)  → 행 수 자동 감지, store_as: "raw_data"
  ↓
VALIDATE(check: "row_count", source: "$raw_data", logic: {min: 1})  → 비어있으면 STOP
  ↓
CLEAR_RANGE(range: "B3:F100000")  → 넉넉한 범위로 기존 데이터 제거
  ↓
WRITE_DATA(source: "$raw_data")  → 전체 행 자동 기록
```
...
19-1. `dynamic_params`에 `type: "lookup"` 또는 `type: "text"` 항목이 **하나도 없는가?** (엔진이 `"date"` 외의 type을 파싱하면 즉시 Pydantic ValidationError 발생 → plan 실행 불가)
20. 가변 행 데이터를 읽는 `READ_COLUMNS`에 `end_row`가 **생략 또는 null**인가? (고정 값이면 매일 행 수가 달라질 때 데이터 누락 또는 초과 읽기 발생)
21. 가변 행 데이터를 쓰는 step이 `WRITE_DATA`를 사용하는가? (고정 range 기반 쓰기는 가변 데이터에 부적합)
...
**⛔ 금지 패턴 — 아래 형태는 Pydantic ValidationError로 plan 실행이 즉시 실패한다:**
```json
// ❌ prompt 누락 → Field required 에러
{"type": "date", "format": "YYYY-MM-DD", "description": "오늘 날짜"}

// ❌ type: "lookup" → Input should be 'date' 에러 (엔진에 lookup 타입 없음)
{"type": "lookup", "prompt": "Sheet1에서 오늘 날짜 열 계산..."}

// ❌ type: "text" → 동일 에러 (엔진에 text 타입 없음)
{"type": "text", "prompt": "사용자 입력값"}

// ❌ format만 있고 prompt 없음 → Field required 에러
{"type": "date", "format": "YYYY-MM-DD", "description": "실행일 날짜(자동 해소)"}
```
