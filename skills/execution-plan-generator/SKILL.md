...
18. `dynamic_params`의 모든 항목에 `prompt` 필드가 있는가? (`prompt`는 **필수**. type=date면 함수 키워드, type=lookup/text면 자연어 지시문)
...
| `prompt` | **O** | **모든 type에서 필수.** type별 용도가 다름 (아래 참조) |
| `format` | - | 기대 출력 형식 (`"YYYY-MM-DD"`, `"json"` 등) |
| `description` | - | 사람용 설명 |
| `default` | - | 해소 실패 시 기본값 |

**prompt 필드는 type에 따라 역할이 다르다 — 반드시 지정해야 한다:**

| type | prompt의 역할 | 예시 |
|------|-------------|------|
| `date` | 내장 날짜 함수 키워드. 아래 16개 중 하나 | `"today"`, `"this_week_monday"` |
| `lookup` | LLM에 전달할 자연어 계산 지시문 | `"Sheet1 Row 3에 BT열부터..."` |
| `text` | LLM에 전달할 자연어 텍스트 생성 지시문 | `"오늘 날짜 기준 보고서 제목"` |

**type=date 허용 prompt 키워드 (16개):**

```
today, yesterday,
this_week_monday, this_week_sunday, last_week_monday, last_week_sunday,
this_month_start, this_month_end, last_month_start, last_month_end,
today_yyyymmdd, today_yyyy_mm_dd, week_number, month_number, year, quarter
```
...
// ✅ 올바름 — prompt 필수
{"type": "date", "prompt": "today", "format": "YYYY-MM-DD", "description": "오늘 날짜"}
```
